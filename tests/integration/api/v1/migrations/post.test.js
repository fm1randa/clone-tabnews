...
async function checkMigrationTableExistence() {
  const { rows: { 0: { exists: isMigrationTableAvailable } } } = await database.query('SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = \'public\' AND table_name = \'pgmigrations\');');
...
test("POST to /api/v1/migrations should execute migrations", async () => {
  ...
  const { rows: migrations } = await database.query('SELECT * FROM pgmigrations');
...
test("POST to /api/v1/migrations should NOT execute migrations more than once", async () => {
  ...
  const firstPostResponse = await fetch("http://localhost:3000/api/v1/migrations", {
    method: "POST"
  });
  ...
  const { rows: migrationsFromFirstQuery } = await database.query('SELECT * FROM pgmigrations');
  ...
  const secondPostResponse = await fetch("http://localhost:3000/api/v1/migrations", {
    method: "POST"
  });
  ...
  const { rows: migrationsFromSecondQuery } = await database.query('SELECT * FROM pgmigrations');
  expect(migrationsFromSecondQuery.length).toEqual(migrationsFromFirstQuery.length);

  // Ensure that executed migrations didn't change between the first and second queries
  expect(migrationsFromSecondQuery.every(migrationFromSecondQuery => {
    const equivalentMigrationFromFirstQuery = migrationsFromFirstQuery.find(
      migrationFromFirstQuery => migrationFromFirstQuery.id === migrationFromSecondQuery.id);
    return equivalentMigrationFromFirstQuery.run_on.getTime() === migrationFromSecondQuery.run_on.getTime();
  })).toBeTruthy();
});